...
do
    ...
    fio --name burst --bs=8192 --filename=/dev/sdb --size=2G --ioengine=libaio --direct=1 --rw=randwrite --time_based --runtime=$BACKGROUNDTIME --rate_iops=$BACKGROUNDIOPS
    fio --name burst --bs=8192 --filename=/dev/sdb --size=2G --ioengine=libaio --direct=1 --rw=randwrite --time_based --iodepth=8 --runtime=$BURSTTIME --rate_iops=$BURSTIOPS
done
